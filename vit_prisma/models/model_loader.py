...
def _create_config_from_hf(hf_config, model_name: str, model_type: ModelType):
    ...
    if model_type == ModelType.VISION or model_type == None:  # VISION
        ...
        # Temporary patch for TIMM converted Huggingface config of vit_base_patch32_224
        else:
            config.d_model = 768
            config.n_layers = 12
            config.n_heads = 12
        ...
        config.patch_size = getattr(hf_config, "patch_size", 16) # changed to 32 from 16
    ...
    if hasattr(hf_config, "projection_dim"):
        ...
        config.n_classes = 1000 # changed from config.d_model
        config.return_type = "class_logits"
